...
exports.showRunPrompt = exports.printStartupDuration = exports.cleanOptions = exports.initEnv = exports.initVaporScriptLangEnv = exports.addConfigFile = exports.PLATFORMS = exports.formatStyleIsolationVersionMessage = void 0;
...
function formatStyleIsolationVersionMessage(version) {
    const versionNumber = Number(version);
    const shouldUpgrade = versionNumber !== 2;
    const message = uni_cli_shared_1.M[shouldUpgrade
...
exports.formatStyleIsolationVersionMessage = formatStyleIsolationVersionMessage;
// uni -p
exports.PLATFORMS = [
    'app',
    'h5',
...
function initVaporScriptLangEnv(inputDir) {
    // 临时通过项目根目录的 .lang 标记开启 Vapor JS/TS 脚本编译链路。
    process.env.UNI_APP_X_VAPOR_SCRIPT_LANG = fs_1.default.existsSync(path_1.default.resolve(inputDir, '.lang'))
        ? 'true'
        : 'false';
}
exports.initVaporScriptLangEnv = initVaporScriptLangEnv;
...
function initEnv(type, options) {
    ...
    const isX = (0, uni_cli_shared_1.isUniAppX)();
    const renderMode = isX ? ((0, uni_cli_shared_1.isUniAppXVapor)() ? '蒸汽模式' : 'VDOM模式') : '';
    console.log(uni_cli_shared_1.M['app.compiler.version'].replace('{version}', process.env.UNI_COMPILER_VERSION +
        `（${isX ? 'uni-app x' : 'vue3'}）` +
        renderMode));
    if ((0, uni_cli_shared_1.isUniAppXVapor)() &&
        (process.env.UNI_PLATFORM === 'app' ||
            process.env.UNI_PLATFORM === 'app-harmony')) {
        console.log(uni_cli_shared_1.M['view.render.compiler.target'].replace('{target}', process.env.UNI_APP_X_DOM2_DYNAMIC === 'true'
            ? uni_cli_shared_1.M['view.render.compiler.target.bytecode']
            : uni_cli_shared_1.M['view.render.compiler.target.nativecode']));
    }
    if (isX && !(0, uni_cli_shared_1.isUniAppXVapor)()) {
        console.log(formatStyleIsolationVersionMessage(process.env.UNI_APP_STYLE_ISOLATION_VERSION || '1'));
    }
