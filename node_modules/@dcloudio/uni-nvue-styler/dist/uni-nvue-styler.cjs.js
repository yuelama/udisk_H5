...
const DOM2_SELECTOR_DOCS_URL = `${DOM2_CSS_DOCS_BASE_URL}/common/selector.html#selector`;
const DOM2_FONT_FACE_DOCS_URL = `${DOM2_CSS_DOCS_BASE_URL}/common/at-rules.html#tips`;
...
const MAX_F32_VALUE$1 = 3.4028234663852886e38;
    ...
    return (v) => {
        ...
        if (match && (allowNegative || value[0] !== '-')) {
            ...
            if (Number.isFinite(milliseconds) &&
                Math.abs(milliseconds) <= MAX_F32_VALUE$1) {
                return { value };
            }
        }
        return {
...
const normalizeAnimationTimingFunctionItem = (v, options) => {
    ...
    if (typeof result.value === 'string' &&
        ...
        if (numbers.some((value) => !Number.isFinite(value) || Math.abs(value) > MAX_F32_VALUE$1)) {
...
const normalizeBackdropFilter = (v) => {
    v = (v || '').toString();
    if (v === 'none') {
        return { value: v };
    }
...
function createRuleProcessor(opts = {}) {
    return (rule, helper) => {
        ...
        rule.selector = rule.selectors
            ...
            const reason = 'ERROR: Selector `' +
                selector +
                '` is not supported. ' +
                type +
                ' only support classname selector';
            rule.warn(helper.result, appendDom2Docs(reason, opts.dom2 ? DOM2_SELECTOR_DOCS_URL : undefined));
            return '';
        })
            .filter(Boolean)
            .join(', ');
...
function normalizeDecl(decl, options) {
    ...
    const atRule = decl.parent?.type === 'atrule' ? decl.parent.name : '';
