    ...
    return function transformIdentifier() {
        if (node.type === compiler_core_1.NodeTypes.INTERPOLATION) {
            ...
            if ((0, uni_cli_shared_1.isUserComponent)(node, context)) {
                ...
                (0, transformComponent_1.rewriteBinding)(node, context, externalClasses, (0, uni_cli_shared_1.isAlipayXStyleIsolation)());
            }
            else {
                rewriteAlipayStyleIsolationClasses(node, context);
            }
            ...
            if (context.isX && isElement) {
                ...
                if (partProp) {
                    ...
                    for (const index of skipIndex) {
                        ...
                        if ((0, uni_cli_shared_1.isDirectiveNode)(prop) && (0, transformClass_1.isClassBinding)(prop)) {
                ...
                if ((0, uni_cli_shared_1.isDirectiveNode)(dir)) {
                    ...
                    if (exp) {
                        if (isBuiltIn(dir)) {
                            ...
                            (0, uni_cli_shared_1.isAlipayXStyleIsolation)() &&
            ...
            if (virtualHost) {
                if (!hasClassBinding) {
                    hasClassBinding = true;
                    const classProp = (0, transformClass_1.createVirtualHostClass)(props, context);
 ...
 */
function rewriteAlipayStyleIsolationClasses(node, context) {
    if (!context.isX ||
        !(0, uni_cli_shared_1.isAlipayXStyleIsolation)() ||
        !context.filename ||
        !(0, uni_cli_shared_1.isElementNode)(node)) {
        return;
    }
    const mask = (0, uni_cli_shared_1.getAlipayStyleIsolationClassMask)(context.filename);
    const hasRuntimeClass = node.props.some((prop) => ((0, uni_cli_shared_1.isDirectiveNode)(prop) &&
        ((0, transformClass_1.isClassBinding)(prop) ||
            (prop.name === 'bind' &&
                prop.arg &&
                (0, uni_cli_shared_1.isSimpleExpressionNode)(prop.arg) &&
                prop.arg.content === 'part'))) ||
        prop.name === 'part') ||
        !!(context.miniProgram.component?.mergeVirtualHostAttributes &&
            context.rootNode === node);
    for (const prop of node.props) {
        if ((0, uni_cli_shared_1.isAttributeNode)(prop) && prop.name === 'class' && prop.value?.content) {
            // mask 为 0 时只做保留前缀校验，实际展开留给后续合并后的动态 class。
            prop.value.content = (0, uni_cli_shared_1.formatAlipayStyleIsolationClasses)(prop.value.content, hasRuntimeClass ? 0 : mask);
        }
        else if ((0, uni_cli_shared_1.isAttributeNode)(prop) &&
            ...
            prop.value.content = (0, uni_cli_shared_1.formatAlipayStyleIsolationClasses)(prop.value.content, mask);
        }
    }
}
function wrapAlipayStyleIsolationClass(prop, context) {
    if (!context.isX || !(0, uni_cli_shared_1.isAlipayXStyleIsolation)() || !prop.exp) {
        return;
    }
    (0, utils_1.addUniViewAutoImportFilter)(context);
    const mask = (0, uni_cli_shared_1.getAlipayStyleIsolationClassMask)(context.filename);
    // 先复用原有表达式重写，再在视图层调用 SJS，避免把 _ctx 等逻辑层标识符输出到模板。
    prop.exp = (0, utils_1.rewriteExpression)((0, compiler_core_1.createCompoundExpression)([
        (0, compiler_core_1.createSimpleExpression)(utils_1.FILTER_MODULE_NAME),
        '.c(',
        prop.exp,
        `,${mask})`,
    ]), context);
}
function wrapAlipayStyleIsolationClassAttribute(prop, context) {
    (0, utils_1.addUniViewAutoImportFilter)(context);
    const mask = (0, uni_cli_shared_1.getAlipayStyleIsolationClassMask)(context.filename);
    const exp = (0, utils_1.rewriteExpression)(prop.exp, context);
...
function isAlipayStyleIsolationClassAttribute(node, prop) {
    if (!(0, uni_cli_shared_1.isElementNode)(node) || node.tagType !== compiler_core_1.ElementTypes.ELEMENT) {
        return false;
    }
    const name = (0, uni_cli_shared_1.isAttributeNode)(prop)
        ? prop.name
        : prop.name === 'bind' &&
            prop.arg &&
            (0, uni_cli_shared_1.isSimpleExpressionNode)(prop.arg) &&
            ...
            ? prop.arg.content
            : '';
    const matched = name === 'hover-class' ||
 ...
 */
function rewriteAlipayStaticExternalClasses(node, context, externalClasses) {
    if (!context.isX || !(0, uni_cli_shared_1.isAlipayXStyleIsolation)() || !externalClasses.length) {
        return;
    }
    const mask = (0, uni_cli_shared_1.getAlipayStyleIsolationClassMask)(context.filename);
    for (const prop of node.props) {
        if ((0, uni_cli_shared_1.isAttributeNode)(prop) &&
            ...
            prop.value?.content) {
            prop.value.content = (0, uni_cli_shared_1.formatAlipayStyleIsolationClasses)(prop.value.content, mask, false);
        }
    }
}
function isAlipayExternalClassBinding(prop, externalClasses) {
    return ((0, uni_cli_shared_1.isAlipayXStyleIsolation)() &&
        prop.name === 'bind' &&
        !!prop.arg &&
        (0, uni_cli_shared_1.isSimpleExpressionNode)(prop.arg) &&
        prop.arg.isStatic &&
...
function wrapAlipayExternalClass(prop, context) {
    (0, utils_1.addUniViewAutoImportFilter)(context);
    const mask = (0, uni_cli_shared_1.getAlipayStyleIsolationClassMask)(context.filename);
    const exp = (0, utils_1.rewriteExpression)(prop.exp, context);
...
function getComponentExternalClasses(source, parentFile, babelParserPlugins) {
    ...
    if ((0, uni_cli_shared_1.isAlipayXStyleIsolation)() && code.includes('#endif')) {
        code = (0, uni_cli_shared_1.preJs)((0, uni_cli_shared_1.preHtml)(code, source), source);
    }
    let scriptContent;
    if (source) {
    ...
    if (program) {
        ...
        const styleIsolation = (0, uni_cli_shared_1.parseStyleIsolation)(program);
        if (styleIsolation) {
            (0, uni_cli_shared_1.updateMiniProgramComponentStyleIsolation)(source, styleIsolation);
        }
        else {
            (0, uni_cli_shared_1.clearMiniProgramComponentStyleIsolation)(source);
        }
