 ...
 */
function rewriteBinding({ tag, props }, context, externalClasses, preserveDynamicExternalClasses = false) {
    ...
    for (let i = 0; i < props.length; i++) {
        ...
        if ((0, uni_cli_shared_1.isAttributeNode)(prop)) {
            ...
            if (context.isX) {
                if (name === 'class') {
                    classExprArr.push((0, types_1.stringLiteral)(prop.value?.content || ''));
                    continue;
                }
                else if (name === 'style') {
                    styleExprArr.push((0, types_1.stringLiteral)(prop.value?.content || ''));
                    continue;
                }
            }
    ...
    if (classExprArr.length) {
        const classValue = classExprArr.length === 1
            ? classExprArr[0]
            : (0, types_1.arrayExpression)(classExprArr);
        properties.push(createObjectProperty('class', (0, types_1.isStringLiteral)(classValue)
            ? classValue
            : createNormalizeExpr(compiler_core_1.NORMALIZE_CLASS, classValue, context)));
    }
    if (styleExprArr.length) {
        const styleValue = styleExprArr.length === 1
            ? styleExprArr[0]
            : (0, types_1.arrayExpression)(styleExprArr);
        properties.push(createObjectProperty('style', (0, types_1.isStringLiteral)(styleValue)
            ? styleValue
            : createNormalizeExpr(compiler_core_1.NORMALIZE_STYLE, styleValue, context)));
    }
...
function createNormalizeExpr(name, args, context) {
    return (0, types_1.callExpression)((0, types_1.identifier)(context.helperString(name)), [args]);
}
