...
function uniMiniProgramPlugin(options) {
    ...
    const normalizedCopyOptions = (0, copy_1.normalizeCopyOptions)(copyOptions, options);
            ...
            miniProgram: {
                ...
                filter: template.filter
                    ? {
                        ...
                        generate: createAutoImportFilterGenerate(template.filter.generate),
                    }
                    : undefined,
            ...
            return {
                ...
                resolve: {
                    alias: {
                        ...
                        ...(0, uni_cli_shared_1.resolveVueI18nAlias)(),
                        // 项目未安装 pinia 时使用内部版本及其依赖
                        ...(0, uni_cli_shared_1.resolvePiniaAlias)(),
                ...
                build: (0, build_1.buildOptions)(options),
            };
        },
        ...
        generateBundle() {
                ...
                if (isX) {
                    if (process.env.UNI_COMPILE_TARGET !== 'uni_modules') {
                        ...
                        if (template.filter.setStyle && !autoImportFilterEmitted) {
                            ...
                            const uniViewSource = fs_extra_1.default.readFileSync(path_1.default.resolve(__dirname, uniViewPath), 'utf8');
                            this.emitFile({
                                type: 'asset',
                                fileName: `common/uniView${extname}`,
                                source: uniViewSource,
                            });
                            (0, independentUtils_1.getIndependentSubPackages)().forEach(({ root }) => {
                                this.emitFile({
                                    type: 'asset',
            ...
            if (!resetCssEmitted) {
                if (isX) {
                    ...
                    const uvueCssSource = genUVueCssCode((0, uni_cli_shared_1.parseManifestJsonOnce)(process.env.UNI_INPUT_DIR));
                    this.emitFile({
                        type: 'asset',
                    ...
                    if (nvueCssPaths && nvueCssPaths.length) {
                        resetCssEmitted = true;
                        const nvueCssSource = (0, uni_cli_shared_1.genNVueCssCode)((0, uni_cli_shared_1.parseManifestJsonOnce)(process.env.UNI_INPUT_DIR));
                        this.emitFile({
                            type: 'asset',
...
function createAutoImportFilterGenerate(generate) {
    return (filter, filename, ownerFilename) => {
        return generate?.(filter, ownerFilename
            ? resolveAutoImportFilterFilename(filter, filename, ownerFilename)
            : filename);
    };
}
function resolveAutoImportFilterFilename(filter, filename, ownerFilename) {
    const inputDir = process.env.UNI_INPUT_DIR;
    if (!inputDir) {
        return filename;
    }
    const resolvedFilename = path_1.default.isAbsolute(ownerFilename)
        ? ownerFilename
        : path_1.default.resolve(inputDir, ownerFilename);
    const independentRoot = (0, independentUtils_1.getIndependentRootByFilename)(resolvedFilename, inputDir);
    if (!independentRoot) {
        return filename;
    }
    const templateFilename = (0, uni_cli_shared_1.removeExt)((0, uni_cli_shared_1.normalizeMiniProgramFilename)(resolvedFilename, inputDir));
    return (0, uni_cli_shared_1.relativeFile)(templateFilename, `${independentRoot}/common/${filter.id}`);
}
