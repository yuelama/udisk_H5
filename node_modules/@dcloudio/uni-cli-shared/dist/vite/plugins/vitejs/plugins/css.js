...
function cssPostPlugin(config, { platform, isJsCode, preserveModules, chunkCssFilename, chunkCssCode, includeComponentCss, emitFile, }) {
    ...
    const cssChunkCodeHashCache = new Map();
    let cssChunks;
    const isDom2 = process.env.UNI_APP_X_DOM2 === 'true';
    const isDom2App = isDom2 && platform.startsWith('app');
    // 仅限定鸿蒙 DOM2 开发模式，避免影响其它平台和生产构建。
    const enableCssIncremental = process.env.NODE_ENV === 'development' &&
    ...
    return {
        ...
        async generateBundle() {
            ...
            for (const filename of cssChunks.keys()) {
                ...
                const cssCodeHash = enableCssIncremental ? (0, utils_3.hash)(cssCode) : '';
                const hasFontFace = enableCssIncremental && /@font-face\b/i.test(cssCode);
