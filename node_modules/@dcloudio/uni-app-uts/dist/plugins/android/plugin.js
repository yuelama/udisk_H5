...
function uniAppPlugin() {
    ...
    const uniXKotlinCompiler = process.env.UNI_APP_X_TSC === 'true'
        ? (0, uni_cli_shared_1.resolveUTSCompiler)().createUniXKotlinCompilerOnce({
            resolveWorkers,
            loadUasmTransformer: (0, uni_cli_shared_1.initUasmTransformerCreator)('app-android'),
            sourceFileCallback: (0, uni_cli_shared_1.initSourceFileCallback)(),
        })
        : null;
    const changedFiles = [];
    ...
    return {
        ...
        async writeBundle() {
            ...
            if (!(0, uni_cli_shared_1.isNormalCompileTarget)()) {
                if (process.env.UNI_COMPILE_TARGET === 'ext-api') {
                    if (uniXKotlinCompiler) {
                        await uniXKotlinCompiler.addRootFile(path_1.default.join(tscOutputDir, entryFileName + '.ts'));
                        await uniXKotlinCompiler.close();
                        const res = await compileApp(path_1.default.join(uvueOutputDir, entryFileName), {
                            pageCount: 0,
                            split: false,
            ...
            if (uniXKotlinCompiler) {
                ...
                const mainFile = path_1.default.join(tscOutputDir, entryFileName + '.ts');
            ...
            const res = await compileApp(path_1.default.join(uvueOutputDir, entryFileName), {
                ...
                inputDir: uvueOutputDir,
                outputDir: outputDir,
                package: (0, uni_cli_shared_1.parseUniXAppAndroidPackage)(manifestJson.appid),
...
function normalizeFilename(filename, isMain = false) {
    if (isMain) {
        return (0, uni_cli_shared_1.resolveMainUtsName)();
    }
