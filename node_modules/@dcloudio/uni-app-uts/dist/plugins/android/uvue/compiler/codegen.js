...
function createCodegenContext(ast, { rootDir = '', targetLanguage = 'kotlin', mode = 'default', prefixIdentifiers = mode === 'module', bindingMetadata = {}, inline = false, sourceMap = false, filename = '', matchEasyCom = shared_1.NOOP, parseUTSComponent = shared_1.NOOP, parseUTSCustomElement = shared_1.NOOP, originalLineOffset = 0, generatedLineOffset = 0, }) {
    const context = {
        rootDir,
        targetLanguage,
        mode,
    ...
    for (let i = 0; i < assets.length; i++) {
        ...
        if (type === 'component') {
            ...
            if (source) {
                ...
                if ((0, uni_shared_1.isAppUVueBuiltInEasyComponent)(id)) {
...
function genNodeListAsArray(nodes, context, withAnyNullableType = false) {
...
function genArrayExpression(node, context) {
    // @ts-expect-error
    const withAnyNullableType = node.__keyName ? (0, shared_1.isOn)(node.__keyName) : false;
    genNodeListAsArray(node.elements, context, withAnyNullableType);
}
