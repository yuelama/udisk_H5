...
function resolveComponentType(node, context, ssr = false) {
    ...
    // binding analysis.
    // fixed by xxxxxx
    if (!utils_1.__BROWSER__ && !isEasyCom) {
...
function buildProps(node, context, props = node.props, isComponent, isDynamicComponent, ssr = false) {
    ...
    const pushMergeArg = (arg) => {
        if (properties.length) {
            mergeArgs.push((0, compiler_core_1.createObjectExpression)(dedupeProperties(properties, isComponent), elementLoc));
            properties = [];
        }
        if (arg)
            mergeArgs.push(arg);
    ...
    for (let i = 0; i < props.length; i++) {
        ...
        if (prop.type === compiler_core_1.NodeTypes.ATTRIBUTE) {
            ...
            if (directiveTransform) {
                ...
                //   shouldUseBlock = true
                // }
                context.onWarn((0, errors_1.createCompilerError)(55 /* ErrorCodes.X_V_CUSTOM_DIRECTIVE */, loc, undefined, ': v-' + name));
            }
        }
    }
    ...
    if (mergeArgs.length) {
        ...
        propsExpression = (0, compiler_core_1.createObjectExpression)(dedupeProperties(properties, isComponent), elementLoc);
    }
    // patchFlag analysis
    if (hasDynamicKeys) {
...
function dedupeProperties(properties, isComponent) {
