...
__ins.emit(event, ...do_not_transform_spread)
    ...
    if (options.componentType !== 'app') {
        const { code, ast, preamble, map } = (0, normalScript_1.processTemplate)(sfc, {
            relativeFilename,
    ...
    if (process.env.UNI_APP_X_DOM2 === 'true') {
        ...
        if (options.className) {
            ...
            if (componentType === 'page' || componentType === 'component') {
                ...
                runtimeOptions += `\n  __className,`;
                runtimeOptions += `\n  __filename: '${(options.templateOptions?.compilerOptions).relativeFilename ||
                    ''}',`;
            }
        }
    }
        ...
        setupPreambleLines.push(`const _ctx = __ins.proxy${options.genDefaultAs
    ...
    if (ctx.optionsRuntimeDecl) {
        const defineOptions = genDefineOptions(scriptSetup.content, ctx.optionsRuntimeDecl);
        if (defineOptions) {
            runtimeOptions += `\n` + defineOptions + `,`;
        }
    }
...
function genDefineOptions(content, node) {
